...
function generateDiscountPrice(price) {
  const discount = 20;
  const discountedPrice = price - (price * discount / 100);
  return Math.ceil(discountedPrice.toFixed(2));
}
...
//                 <span class="dRObSZ text-secondary fw-bold" style="text-s">Select up to ${maxAddons}</span>`;
//     }
...
//                         <div class="col-md-8" style="padding: 0 !important; flex: 1;">
//                           +&#8377;${parsedPrice}
//                         </div>
//                         <div class="col-md-2" style="flex-shrink: 0;">
...
function generateAddons(addons, selectedItem = false) {
  ...
  for (const addon of addons) {
    ...
    if (maxAddons != choices.length) {
      temp += `<span class="ldyIml"></span>
               <span class="dRObSZ text-secondary">Select up to ${maxAddons}</span>`;
    }
      ...
      temp += `
        <div style="padding: 12px 0; border-bottom: 1px solid #eee;">
          <div style="display: flex; flex-wrap: wrap; justify-content: space-between;">
            ...
            <div style="flex: 1 1 30%; display: flex; align-items: center; justify-content: flex-end;">
              <div style="font-size: 14px; margin-right: 8px;"><strong>+₹${parsedPrice}</strong></div>
              <input kind="CHECKBOX" class="meAlJ addons" type="checkbox"
                name="${id}" id="${id}"
                data-group-id="${groupId}" data-variant-id="${id}"
                data-max="${maxAddons}" data-price="<strong>₹${parsedPrice}</strong>">
            </div>
          </div>
        </div>`;
    }
      ...
      if (Array.isArray(variations)) {
        for (const variation of variations) {
          ...
          if (parsedPrice && variantGroups.length == index + 1) {
            div += `<span class="item-name-price"><div class="item-price-data">&#8377;<s>${parsedPrice}</s> ₹${generateDiscountPrice(parsedPrice)} </div></span>`
          }
          div += `</div>`
...
function calculateVariantTotal() {
  ...
  basePrice = parseFloat(generateDiscountPrice(generatePrice(basePrice)))
  ...
  console.log(typeof basePrice, typeof addonsCost)

  let totalPrice = (basePrice + generatePrice(addonsCost)) * quantity;
...
$(document).ready(function () {
  ...
  $("#HomePageSignUpBoxOpenButton2").click(function () {
  ...
  $("#HomePageSignInMobileBoxOpenButton").click(function () {

    $("#HomePageSignUpBoxBox").show();
  });

  $("#HomePageSignInBoxOpenButton2").click(function () {
